...
# sbx logs
flynn -c sbx-qa-us-east-1 -a "${APP_NAME}" log > "${LOGS_DIR}/sbx.log" 2>/dev/null

# qa logs
flynn -c qa-us-east-1 -a "${APP_NAME}" log > "${LOGS_DIR}/qa.log" 2>/dev/null

# prod logs
flynn -c prod-us-east-1 -a "${APP_NAME}" log > "${LOGS_DIR}/prod.log" 2>/dev/null
